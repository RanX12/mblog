(function () {
  ...
  window.App.clock = {
    install: function () {
      ...
      let that = this
      const requestUrl = "https://wttr.in/" + returnCitySN.cip + '?format="%l+\\+%c+\\+%t+\\+%h"'
      fetch(requestUrl)
        .then((city) => city.text())
        .then((city) => {
    ...
    clock() {
      ...
      var date = new Date()
      var currentHours = processDate(date.getHours(), 2) + ":" + processDate(date.getMinutes(), 2) + ":" + processDate(date.getSeconds(), 2)
      var currentYear = processDate(date.getFullYear(), 4) + "-" + processDate(date.getMonth() + 1, 2) + "-" + processDate(date.getDate(), 2) + " " + weekNumber[date.getDay()]
      var hour = date.getHours()

      if ((hour > 12 ? ((hour -= 12), (morningOrAfternoon = " PM")) : (morningOrAfternoon = " AM") && document.getElementById("card-clock-time"))) {
        ...
        document.getElementById("card-clock-dackorlight").innerHTML = morningOrAfternoon
      }
    }
  };
}.call(this));
